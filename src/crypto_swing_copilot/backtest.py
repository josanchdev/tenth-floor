...
Usage::
    ...
    python -m crypto_swing_copilot.backtest --days 90 --profile validation
    python -m crypto_swing_copilot.backtest --days 30 --min-rr 1.8
    python -m crypto_swing_copilot.backtest --days 60 --no-volume-gate
    python -m crypto_swing_copilot.backtest --days 90 --no-capitulation  # disable bypass
"""
...
def _fetch_fg_history(days: int) -> dict[str, int]:
    ...
    try:
        req = urllib.request.Request(
            url,
            headers={"User-Agent": "crypto-swing-copilot-backtest/0.1"},
        )
